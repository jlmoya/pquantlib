...
**Started:** 2026-05-29 · **Closed:** 2026-05-31
**Predecessor:** `pquantlib-phase10-complete` @ `d3746e4` — 2652/0/0
**Final:** `pquantlib-phase11-complete` + `pquantlib-100-complete` — **4048/0/0**
**C++ ground truth:** QuantLib v1.42.1 @ `099987f0`

**Phase 11 was the full-closure mega-phase: 12 sequential waves, 2652 → 4048 tests (+1396), ~520 classes, ~50 subagent clusters.** It took PQuantLib from "vanilla + opt-in extensions" to functional 1:1 with the C++ v1.42.1 surface — every C++ class either ported or documented (representation-mismatch / superseded-legacy / permanently-delegated). See [`docs/carve-outs.md`](../carve-outs.md) Statistics for the final accounting.
...
| W12 | `pquantlib-phase11-complete` + `pquantlib-100-complete` @ `215ea1b`+ | +69 | **4048** | 2026-05-31 |
...
## W12 — coverage audit + core cashflows gap-fill + final closure

Mini-pilot (W12-A CMS) + parallel (W12-C dividend/BMA/overnight), then W12-B (CappedFloored/Digital) off the merged base, then inline final closure. +69 tests / 3979 → 4048. Tags `pquantlib-phase11-complete` + `pquantlib-100-complete`.

W12 began with a programmatic coverage audit (`migration-harness/check_coverage.py`) that triaged the raw name-match gap into 4 buckets (real-gaps / representation-mismatch / superseded-legacy / permanently-delegated — see `docs/carve-outs.md` Statistics). Only bucket 1 (core `cashflows`) was a genuine omission from L2-D, and W12 filled it:

| Cluster | Tests | Scope |
|---|---|---|
| W12-A CMS coupons + Hagan pricers | +30 | CmsCoupon + CmsLeg + CmsCouponPricer + GFunction family + GFunctionFactory + HaganPricer + AnalyticHaganPricer + NumericHaganPricer + ConundrumIntegrand + LognormalCmsSpreadPricer (closes the W8-A CMS-spread deferral) |
| W12-C dividend + BMA + overnight pricers | +17 | Dividend + FixedDividend + FractionalDividend + BMAIndex + AverageBMACoupon/Leg/Pricer + CompoundingOvernightIndexedCouponPricer + ArithmeticAveragedOvernightIndexedCouponPricer |
| W12-B CappedFloored + Digital coupons | +22 | CappedFlooredCoupon/Ibor/Cms/Overnight + DigitalCoupon/Ibor/Cms + DigitalReplication + StrippedCappedFlooredCoupon; **upgraded the L2-D BlackIborCouponPricer stub (which raised) into a real optionlet pricer** |

**Final-closure (inline):** carve-outs.md rebaseline (MarketModels → CLOSED; Statistics rewritten with the 4-bucket audit; the 50.6% raw name-match floor vs the 112%-of-jquantlib-final functional reality explained); the audit script committed as W12 tooling; full 8-step doc sweep; terminal tags.

**W12 divergences / deferrals:** the equity-cashflow sub-batch (EquityCashFlow + EquityIndex + QuantoTermStructure) was **discarded mid-W12** — the subagent was cut off by a transient server rate-limit with the batch untested + carrying a real bug; rather than commit untested code (violating the TDD discipline), it was deferred to a documented follow-up. CMS/overnight cap-floor *pricing* (vs the uncapped/intrinsic paths, which work) needs a Hagan-replication / Black-overnight caplet pricer — deferred.

## Phase 11 grand total

2652 → 4048 (**+1396 tests**) across 12 waves / ~520 classes / ~50 subagent clusters / 2-4 sessions. Closed: every specialty model (MarkovFunctional, Bates variants, Heston SLV, Gaussian1d), the entire `experimental/*` tree (credit/exotic/FD/volatility/math/processes/commodities/inflation/long-tail), the **entire MarketModels/BGM/LMM domain** (W9–W11, two canonical end-to-end tests passing), and the core cashflows CMS/CappedFloored/Digital families (W12). Net effect: **functional 1:1 with C++ QuantLib v1.42.1** — the `pquantlib-100-complete` tag. Per the delegation philosophy, tooling-boundary items (historical-vol estimators, GARCH) remain permanently-delegated to numpy/scipy/arch by design, not ported.
