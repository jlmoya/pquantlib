...
- **Phase:** **Phase 3 complete** — all 5 L3 clusters closed and tagged `pquantlib-phase3-complete` @ `aacc2c2`.
- **Branch:** `main`. No active feature branches. Workflow for the next phase will mirror Phase 1+2+3: per-cluster worktrees, push direct to `main` per cluster (FF-only).
- **Workspace:** uv-managed 4-package monorepo (`pquantlib` + `pquantlib-{contrib,helpers,samples}`). Dependencies: numpy, scipy, pytest, pyright, ruff. No new deps in Phase 3.
- **Python:** 3.14. **Type checker:** pyright strict. **Lint+format:** ruff. **Test framework:** pytest 9+, **currently 1284/0/0**.
...
- **L2 layer landed (Phase 2):** quotes (Quote/SimpleQuote/Derived/Composite); termstructures core (TermStructure + Extrapolator + BootstrapHelper + Pillar enum); 4 cross-cluster Protocols (Yield/Ibor/Overnight/Swap); Index + IndexManager; Compounding enum; InterestRate class; YieldTermStructure + FlatForward + Interpolated{Zero,Forward,Discount} + 3 spreaded + Implied; InterestRateIndex/IborIndex/OvernightIndex/SwapIndex abstracts + 8 ibor concretes + 2 swap indexes + 7 rate helpers; CashFlow + Coupon hierarchy + FixedRateCoupon + IborCoupon + OvernightIndexedCoupon + 3 leg generators + 3 coupon pricers + CashFlows aggregator + Duration enum; VolatilityTermStructure + SmileSection + BlackVol/LocalVol Constant/Curve/Surface.
- **L3 layer landed (Phase 3):** Settings.evaluation_date observable (+ 4 retroactive L1/L2 cleanups); Payoff hierarchy (PlainVanilla + Cash/AssetOrNothing + Gap + SuperFund + SuperShare); Exercise hierarchy (European/American/Bermudan); Instrument + PricingEngine + GenericEngine[ArgsT, ResultsT]; BlackFormula family (lognormal + bachelier + implied-vol + Black-vega); Option + OneAssetOption + 3 cross-cluster Protocols (Instrument / PricingEngine / StochasticProcess); Bond abstract + 4 concretes (Fixed/Zero/Floating/AmortizingFixed) + Callability + DiscountingBondEngine + BondForward; Swap abstract + VanillaSwap + OIS + ZeroCoupon + make_vanilla_swap + make_ois + DiscountingSwapEngine; StochasticProcess hierarchy + GBSM family + VanillaOption + EuropeanOption + AnalyticEuropeanEngine + BinomialVanillaEngine + BlackCalculator; Forward abstract + Position + FxForward + ForwardRateAgreement + DiscountingFwdEngine. All L2 carry-overs (SwapRateHelper/OISRateHelper/SwapIndex/FraRateHelper) closed.
- **Parallelization wins:** Phase 1 (~25 min), Phase 2 (~35 min), Phase 3 (~50 min) all via subagent fan-out off a sequential pilot. Cross-cluster Protocols proved out as integration glue across all 3 phases. Pattern documented in `phase{1,2,3}-completion.md`.
- **Cumulative L1+L2+L3 carve-outs** (deferred): L1 carry-overs (GaussianOrthogonalPolynomial, Sobol/Burley2020, LM/BFGS/Simplex, 8+ cubic splines, QR/Eigen/SVD/SparseMatrix, GammaFunction); L2 carry-overs (all inflation, all credit, ZABR/SABR/XABR vol, capfloor/optionlet/swaption vol, 35 specialty ibors, specialized cashflows, advanced curve construction); L3 carve-outs (all exotic instruments, Swaption + CapFloor, CDS + ConvertibleBond, specialty swaps, all model-coupled engines, all MC/FD engines, specialty processes, lattice/tree hierarchy, VanillaOption.implied_volatility). Full lists in `phase{1,2,3}-completion.md`.
- **Next phase:** **Phase 4 (L4)** — models, mirroring jquantlib's `phase2-L4-models-plan.md`. Short-rate models + Heston + Hull-White + G2 + calibration engines + the full optimizer suite. Will unblock many Phase 3 carve-outs.
