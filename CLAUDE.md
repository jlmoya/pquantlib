...
- **Phase:** **Phase 12 COMPLETE — `pquantlib-siblings-complete`.** The three JQuantLib sibling Maven modules are ported into the workspace members `pquantlib-contrib` / `pquantlib-helpers` / `pquantlib-samples` (5 sequenced waves W-S1…W-S5). API-era reconciliation: the siblings targeted ~QL-1.0; PQuantLib **core stays pure v1.42.1**, so period-specific retired-API classes (dividend-option compat + the pre-1.0 FD framework) are hosted in `pquantlib-helpers`, while the ConvertibleBond subsystem (genuine v1.42.1) closed its carve-out in **core** (W-S4). Sibling test state **4237 passed / 0 failed / 3 skipped** (the 3 PENDING samples). Predecessor Phase 11 (`pquantlib-100-complete` / `pquantlib-final` @ `1fdb1db`, 4048/0/0) remains functional 1:1 with C++ v1.42.1. See [`docs/migration/phase12-completion.md`](docs/migration/phase12-completion.md) + [`docs/migration/phase11-completion.md`](docs/migration/phase11-completion.md) + [`docs/carve-outs.md`](docs/carve-outs.md) Statistics.
- **Branch:** `main` (Phase 12 closed). No active feature branches.
- **Workspace:** uv-managed 4-package monorepo (all 4 members now populated: `pquantlib` core + `pquantlib-contrib` + `pquantlib-helpers` + `pquantlib-samples`). Dependencies: numpy, scipy, pytest, pyright, ruff (+ matplotlib opt-in for the SobolChart sample, env-guarded). No new core deps in Phase 12.
- **Python:** 3.14. **Type checker:** pyright strict. **Lint+format:** ruff. **Test framework:** pytest 9+, **main currently 4237 passed / 0 failed / 3 skipped**.
...
- **Planned-migration milestone (Phase 6):** the end of the originally-planned migration — Phases 0-6 closed; ~340 classes ported across ~1958 tests (54.2% of jquantlib-final 3610); vanilla pricing + calibration end-to-end + American MC + analytic exotics covered. Preserved under **`pquantlib-phase6-complete`** @ `998fed3`. _The `pquantlib-final` tag originally marked this milestone; on 2026-05-31 it was re-pointed to the Phase 11 terminal (`1fdb1db`) to denote the true end of the project (functional 1:1, 4048/0/0)._
...
- **Sibling packages landed (Phase 12):** the three JQuantLib sibling modules → workspace members. **W-S1** `XorShiftRandom` → `pquantlib-contrib` (JQuantLib-original, no C++ equiv; TIGHT vs Java); **W-S2** dividend-option compat primitives (`DividendVanillaOption` + `BinomialDividendVanillaEngine` + `BlackScholesDividendLattice`) + **W-S3** 6 helper builders + the retired pre-1.0 FD framework (~35 classes: `TridiagonalOperator`/`BSMOperator`/D-ops + `FiniteDifferenceModel`/schemes + boundary/step conditions + `FDVanillaEngine`→`FDDividend{European,American}Engine` + `SampledCurve`) → `pquantlib-helpers` (retired-API hosted in helpers, built on v1.42.1 primitives, TIGHT vs Java — reproduces the Java `FDMultiPeriodEngine` (gridPoints,timeSteps) param-swap + the lattice dividend-drop bug for parity); **W-S4** ConvertibleBond subsystem (`Convertible{FixedCoupon,ZeroCoupon}Bond` + `SoftCallability` + `DiscretizedConvertible` (Tsiveriotis–Fernandes) + `TsiveriotisFernandesLattice` + `BinomialConvertibleEngine`) → pquantlib **core**, closing the carve-out (TIGHT vs C++ N=801); **W-S5** sample programs + `AllSamples` runner + smoke suite → `pquantlib-samples` (12 COMPLETE + 1 INCOMPLETE [equity_options] + 3 PENDING [bonds/replication/cox_ross_with_hull_white]). Tag `pquantlib-siblings-complete`. ~45 classes / +189 tests / +3 skips.
