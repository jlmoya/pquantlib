...
- **Phase:** **Phase 11 in progress — W1–W8 closed.** Latest tag `pquantlib-phase11-w8-complete` @ `8573dd9` (3763/0/0). W8 (long-tail experimental: CMS-spread/irregular-swaptions/callable+cat bonds/CreditRiskPlus/VannaVolga[closes W4-C]/Heston-Asian-fwd/CLV/generalized-short-rate/xccy-helpers) merged — **experimental/* tree essentially complete.** **Next: W9** (marketmodels/products + callability) per [`docs/migration/phase11-plan.md`](docs/migration/phase11-plan.md). Remaining: W9–W11 marketmodels (~125 files), W12 audit + core-CmsCoupon gap-fill + permanently-delegated re-baseline + final tag.
- **Branch:** `main` @ `8573dd9`. No active feature branches between waves.
- **Workspace:** uv-managed 4-package monorepo. Dependencies: numpy, scipy, pytest, pyright, ruff. No new deps in Phase 11.
- **Python:** 3.14. **Type checker:** pyright strict. **Lint+format:** ruff. **Test framework:** pytest 9+, **main currently 3763/0/0**.
