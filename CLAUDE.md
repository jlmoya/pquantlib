...
- **Phase:** **Phase 2 complete** — all 5 L2 clusters closed and tagged `pquantlib-phase2-complete` @ `b5d2519`.
- **Branch:** `main`. No active feature branches. Workflow for the next phase will mirror Phase 1+2: per-cluster worktrees, push direct to `main` per cluster (FF-only).
- **Workspace:** uv-managed 4-package monorepo (`pquantlib` + `pquantlib-{contrib,helpers,samples}`). Dependencies: numpy, scipy, pytest, pyright, ruff. No new deps in Phase 2.
- **Python:** 3.14. **Type checker:** pyright strict. **Lint+format:** ruff. **Test framework:** pytest 9+, **currently 922/0/0**.
...
- **L2 layer landed (Phase 2):** quotes (Quote/SimpleQuote/Derived/Composite); termstructures core (TermStructure + Extrapolator + BootstrapHelper + Pillar enum); 4 cross-cluster Protocols (Yield/Ibor/Overnight/Swap); Index + IndexManager; Compounding enum; InterestRate class; YieldTermStructure + FlatForward + Interpolated{Zero,Forward,Discount} + 3 spreaded + Implied; InterestRateIndex/IborIndex/OvernightIndex/SwapIndex abstracts + 8 ibor concretes (Euribor/USDLibor/GBPLibor/Eonia/Sofr/Sonia/FedFunds/Estr) + 2 swap indexes + 7 rate helpers; CashFlow + Coupon hierarchy + FixedRateCoupon + IborCoupon + OvernightIndexedCoupon + 3 leg generators + 3 coupon pricers + CashFlows aggregator + Duration enum; VolatilityTermStructure + SmileSection + BlackVol/LocalVol Constant/Curve/Surface (equity/FX).
- **Parallelization wins:** Phase 1 (Stage 4 calendars + L1-B/C/D/E ~25 min) and Phase 2 (L2-B/C/D/E ~35 min) both via subagent fan-out. Cross-cluster Protocols proved out as integration glue at L2 merge time. Pattern documented in `phase1-completion.md` + `phase2-completion.md`.
- **Cumulative L1+L2 carve-outs** (deferred): from L1 — full GaussianOrthogonalPolynomial hierarchy, Sobol/Burley2020 low-discrepancy, LM/BFGS/Simplex optimizers, 8+ cubic spline variants, QR/Eigen/SVD/SparseMatrix utilities, GammaFunction. From L2 — all inflation, all credit, ZABR/SABR/XABR vol, capfloor/optionlet/swaption vol, 35 specialty ibors, specialized cashflows (Digital/Cms/CapFloored/AverageBMA), advanced curve construction (FittedBondDiscountCurve / MultiCurve / GlobalBootstrap / spline-fitting variants), `Settings.evaluation_date` observable wiring. Full list in `phase1-completion.md` + `phase2-completion.md`.
- **Next phase:** **Phase 3 (L3)** — instruments + pricingengines, mirroring jquantlib's `phase2-L3-instruments-pricingengines-plan.md`. Likely largest L-layer.
