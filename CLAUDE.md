...
- **Phase:** **Phase 11 in progress — W1–W7 closed.** Latest tag `pquantlib-phase11-w7-complete` @ `6ae1cfd` (3620/0/0). W7 (experimental processes + commodities full energy-instrument stack + experimental inflation YoY-optionlet-stripping + variance-gamma + FFT engines) merged. **Next: W8** (long-tail experimental: coupons/swaptions/callablebonds/catbonds/mcbasket/asian/fx/forward/basismodels/averageois/risk/lattices/models/shortrate/termstructures) per [`docs/migration/phase11-plan.md`](docs/migration/phase11-plan.md).
- **Branch:** `main` @ `6ae1cfd`. No active feature branches between waves.
- **Workspace:** uv-managed 4-package monorepo. Dependencies: numpy, scipy, pytest, pyright, ruff. No new deps in Phase 11.
- **Python:** 3.14. **Type checker:** pyright strict. **Lint+format:** ruff. **Test framework:** pytest 9+, **main currently 3620/0/0**.
