...
- **Phase:** **Phase 11 COMPLETE — `pquantlib-phase11-complete` + `pquantlib-100-complete` + `pquantlib-final` (re-pointed here from the Phase 6 close).** All 12 waves closed; terminal commit `1fdb1db` (4048/0/0). Functional 1:1 with C++ QuantLib v1.42.1: every C++ class either ported or documented (representation-mismatch / superseded-`legacy/`-LFM / permanently-delegated-to-numpy/scipy/arch). Full marketmodels/BGM/LMM domain done (W9–W11, 2 canonical end-to-end tests pass); core cashflows CMS/CappedFloored/Digital families filled (W12). Test parity 4048/3610 = 112% of jquantlib-final. See [`docs/migration/phase11-completion.md`](docs/migration/phase11-completion.md) + [`docs/carve-outs.md`](docs/carve-outs.md) Statistics.
- **Branch:** `main` (Phase 11 closed). No active feature branches. No active feature branches between waves.
- **Workspace:** uv-managed 4-package monorepo. Dependencies: numpy, scipy, pytest, pyright, ruff. No new deps in Phase 11.
- **Python:** 3.14. **Type checker:** pyright strict. **Lint+format:** ruff. **Test framework:** pytest 9+, **main currently 4048/0/0**.
