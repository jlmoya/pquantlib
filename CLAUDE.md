...
- **Phase:** **Phase 11 in progress — W1–W11 closed.** Latest tag `pquantlib-phase11-w11-complete` @ `1ebbd5c` (3979/0/0). W11 closed the entire ~111-file marketmodels (BGM/LMM) domain (products + callability + pathwise greeks); TWO canonical end-to-end tests (Black caplet + Bermudan-swaption LS) pass. **Next: W12 — the FINAL wave** (coverage-audit triage + cashflows gap-fill [CmsCoupon/CappedFloored/Digital/GFunction-Hagan — also unblocks W8-A CMS-spread] + permanently-delegated re-baseline of `docs/carve-outs.md` + final tags `pquantlib-phase11-complete` + `pquantlib-100-complete`).
- **Branch:** `main` @ `1ebbd5c`. No active feature branches between waves.
- **Workspace:** uv-managed 4-package monorepo. Dependencies: numpy, scipy, pytest, pyright, ruff. No new deps in Phase 11.
- **Python:** 3.14. **Type checker:** pyright strict. **Lint+format:** ruff. **Test framework:** pytest 9+, **main currently 3979/0/0**.
