...
- **Phase:** **Phase 5 complete** — all 5 L5 clusters closed and tagged `pquantlib-phase5-complete` @ `d322fca`.
- **Branch:** `main`. No active feature branches. Workflow for the next phase will mirror Phase 1-5: per-cluster worktrees, push direct to `main` per cluster (FF-only).
- **Workspace:** uv-managed 4-package monorepo. Dependencies: numpy, scipy, pytest, pyright, ruff. No new deps in Phase 5.
- **Python:** 3.14. **Type checker:** pyright strict. **Lint+format:** ruff. **Test framework:** pytest 9+, **currently 1883/0/0**.
...
- **Next phase:** **Phase 6 (modernization sweep + final closure)** — Python 3.14 modernization (PEP 695 generics, match-case, frozen+slots dataclasses, type statements), remaining high-impact carve-outs (LongstaffSchwartz American MC, Heston MC, DoubleBarrier), `pquantlib-final` tag.
