"""Cross-validate United Kingdom holidays for 2020-2030 against the C++ probe.

Probe key: time/calendars/all -> "united_kingdom"
"""
...
def test_name_matches_cpp(cpp: dict[str, Any]) -> None:
    assert UnitedKingdom().name() == cpp["united_kingdom"]["name"]


def test_holidays_match_cpp(cpp: dict[str, Any]) -> None:
    cal = UnitedKingdom()
    expected = {
        Date.from_ymd(int(h["d"]), Month(int(h["m"])), int(h["y"])) for h in cpp["united_kingdom"]["holidays"]
    }
    actual: set[Date] = set()
