"""Cross-validate Argentina holidays for 2020-2030 against the C++ probe.

Probe key: time/calendars/all -> "argentina"
"""
...
def test_name_matches_cpp(cpp: dict[str, Any]) -> None:
    assert Argentina().name() == cpp["argentina"]["name"]


def test_holidays_match_cpp(cpp: dict[str, Any]) -> None:
    cal = Argentina()
    expected = {
        Date.from_ymd(int(h["d"]), Month(int(h["m"])), int(h["y"])) for h in cpp["argentina"]["holidays"]
    }
    actual: set[Date] = set()
