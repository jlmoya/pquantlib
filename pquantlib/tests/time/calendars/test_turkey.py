"""Cross-validate Turkey holidays for 2020-2030 against the C++ probe.

Probe key: time/calendars/all -> "turkey"
"""
...
def test_name_matches_cpp(cpp: dict[str, Any]) -> None:
    assert Turkey().name() == cpp["turkey"]["name"]


def test_holidays_match_cpp(cpp: dict[str, Any]) -> None:
    cal = Turkey()
    expected = {
        Date.from_ymd(int(h["d"]), Month(int(h["m"])), int(h["y"])) for h in cpp["turkey"]["holidays"]
    }
    actual: set[Date] = set()
