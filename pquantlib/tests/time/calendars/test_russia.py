"""Cross-validate Russia (Russian settlement) holidays for 2020-2030 against the C++ probe.

Probe key: time/calendars/all -> "russia"
"""
...
def test_name_matches_cpp(cpp: dict[str, Any]) -> None:
    assert Russia().name() == cpp["russia"]["name"]


def test_holidays_match_cpp(cpp: dict[str, Any]) -> None:
    cal = Russia()
    expected = {
        Date.from_ymd(int(h["d"]), Month(int(h["m"])), int(h["y"])) for h in cpp["russia"]["holidays"]
    }
    actual: set[Date] = set()
