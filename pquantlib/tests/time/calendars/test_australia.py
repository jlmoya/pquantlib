"""Cross-validate Australia holidays for 2020-2030 against the C++ probe.

Probe key: time/calendars/all -> "australia"
"""
...
def test_name_matches_cpp(cpp: dict[str, Any]) -> None:
    assert Australia().name() == cpp["australia"]["name"]


def test_holidays_match_cpp(cpp: dict[str, Any]) -> None:
    cal = Australia()
    expected = {
        Date.from_ymd(int(h["d"]), Month(int(h["m"])), int(h["y"])) for h in cpp["australia"]["holidays"]
    }
    actual: set[Date] = set()
