"""Cross-validate Switzerland holidays for 2020-2030 against the C++ probe.

Probe key: time/calendars/all -> "switzerland"
"""
...
def test_name_matches_cpp(cpp: dict[str, Any]) -> None:
    assert Switzerland().name() == cpp["switzerland"]["name"]


def test_holidays_match_cpp(cpp: dict[str, Any]) -> None:
    cal = Switzerland()
    expected = {
        Date.from_ymd(int(h["d"]), Month(int(h["m"])), int(h["y"])) for h in cpp["switzerland"]["holidays"]
    }
    actual: set[Date] = set()
