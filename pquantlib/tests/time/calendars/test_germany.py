"""Cross-validate Germany holidays for 2020-2030 against the C++ probe.

Probe key: time/calendars/all -> "germany" (default Frankfurt Stock Exchange).
"""
...
from pquantlib.time.calendars.germany import Germany
...
def test_name_matches_cpp(cpp: dict[str, Any]) -> None:
    assert Germany().name() == cpp["germany"]["name"]


def test_holidays_match_cpp(cpp: dict[str, Any]) -> None:
    cal = Germany()
    expected = {
        Date.from_ymd(int(h["d"]), Month(int(h["m"])), int(h["y"])) for h in cpp["germany"]["holidays"]
    }
    actual: set[Date] = set()
