"""Cross-validate Canada holidays for 2020-2030 against the C++ probe.

Probe key: time/calendars/all -> "canada"
"""
...
def test_name_matches_cpp(cpp: dict[str, Any]) -> None:
    assert Canada().name() == cpp["canada"]["name"]


def test_holidays_match_cpp(cpp: dict[str, Any]) -> None:
    cal = Canada()
    expected = {
        Date.from_ymd(int(h["d"]), Month(int(h["m"])), int(h["y"])) for h in cpp["canada"]["holidays"]
    }
    actual: set[Date] = set()
