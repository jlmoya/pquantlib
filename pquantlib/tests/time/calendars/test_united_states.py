"""Cross-validate United States (Settlement) holidays for 2020-2030.

Probe key: time/calendars/all -> "united_states"
"""
...
def test_name_matches_cpp(cpp: dict[str, Any]) -> None:
    assert UnitedStates().name() == cpp["united_states"]["name"]


def test_holidays_match_cpp(cpp: dict[str, Any]) -> None:
    cal = UnitedStates()
    expected = {
        Date.from_ymd(int(h["d"]), Month(int(h["m"])), int(h["y"])) for h in cpp["united_states"]["holidays"]
    }
    actual: set[Date] = set()
