"""Cross-validate CzechRepublic holidays for 2020-2030 against the C++ probe.

Probe key: time/calendars/all -> "czech_republic"
"""
...
def test_name_matches_cpp(cpp: dict[str, Any]) -> None:
    assert CzechRepublic().name() == cpp["czech_republic"]["name"]


def test_holidays_match_cpp(cpp: dict[str, Any]) -> None:
    cal = CzechRepublic()
    expected = {
        Date.from_ymd(int(h["d"]), Month(int(h["m"])), int(h["y"])) for h in cpp["czech_republic"]["holidays"]
    }
    actual: set[Date] = set()
