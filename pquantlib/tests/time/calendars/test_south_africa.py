"""Cross-validate South Africa holidays for 2020-2030 against the C++ probe.

Probe key: time/calendars/all -> "south_africa"
"""
...
def test_name_matches_cpp(cpp: dict[str, Any]) -> None:
    assert SouthAfrica().name() == cpp["south_africa"]["name"]


def test_holidays_match_cpp(cpp: dict[str, Any]) -> None:
    cal = SouthAfrica()
    expected = {
        Date.from_ymd(int(h["d"]), Month(int(h["m"])), int(h["y"])) for h in cpp["south_africa"]["holidays"]
    }
    actual: set[Date] = set()
