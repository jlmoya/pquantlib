"""Cross-validate Israel (Tel Aviv stock exchange) holidays for 2020-2030 against the C++ probe.

Probe key: time/calendars/all -> "israel"
"""
...
from pquantlib.time.calendars.israel import Israel
...
def test_name_matches_cpp(cpp: dict[str, Any]) -> None:
    assert Israel().name() == cpp["israel"]["name"]


def test_holidays_match_cpp(cpp: dict[str, Any]) -> None:
    cal = Israel()
    expected = {
        Date.from_ymd(int(h["d"]), Month(int(h["m"])), int(h["y"])) for h in cpp["israel"]["holidays"]
    }
    actual: set[Date] = set()
