"""Cross-validate Slovenia holidays for 2020-2030 against the C++ v1.43 probe.

Probe key: time/calendars/all -> "slovenia". This calendar is new in C++ v1.43.
"""
...
from pquantlib.time.calendars.slovenia import Slovenia
...
def test_name_matches_cpp(cpp: dict[str, Any]) -> None:
    assert Slovenia().name() == cpp["slovenia"]["name"]
...
def test_holidays_match_cpp(cpp: dict[str, Any]) -> None:
    ...
    expected = {
        ...
        for h in cpp["slovenia"]["holidays"]
    }
    actual: set[Date] = set()
