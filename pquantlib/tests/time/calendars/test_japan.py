"""Cross-validate Japan holidays for 2020-2030 against the C++ probe.

Probe key: time/calendars/all -> "japan"
"""
...
def test_name_matches_cpp(cpp: dict[str, Any]) -> None:
    assert Japan().name() == cpp["japan"]["name"]


def test_holidays_match_cpp(cpp: dict[str, Any]) -> None:
    cal = Japan()
    expected = {Date.from_ymd(int(h["d"]), Month(int(h["m"])), int(h["y"])) for h in cpp["japan"]["holidays"]}
    actual: set[Date] = set()
