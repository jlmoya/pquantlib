"""Cross-validate New Zealand holidays for 2020-2030 against the C++ probe.

Probe key: time/calendars/all -> "new_zealand"
"""
...
def test_name_matches_cpp(cpp: dict[str, Any]) -> None:
    assert NewZealand().name() == cpp["new_zealand"]["name"]


def test_holidays_match_cpp(cpp: dict[str, Any]) -> None:
    cal = NewZealand()
    expected = {
        Date.from_ymd(int(h["d"]), Month(int(h["m"])), int(h["y"])) for h in cpp["new_zealand"]["holidays"]
    }
    actual: set[Date] = set()
