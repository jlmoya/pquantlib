"""Cross-validate Poland holidays for 2020-2030 against the C++ probe.

Probe key: time/calendars/all -> "poland" (default Settlement).
"""
...
from pquantlib.time.calendars.poland import Poland
...
def test_name_matches_cpp(cpp: dict[str, Any]) -> None:
    assert Poland().name() == cpp["poland"]["name"]


def test_holidays_match_cpp(cpp: dict[str, Any]) -> None:
    cal = Poland()
    expected = {
        Date.from_ymd(int(h["d"]), Month(int(h["m"])), int(h["y"])) for h in cpp["poland"]["holidays"]
    }
    actual: set[Date] = set()
