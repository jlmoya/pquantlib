"""Cross-validate Botswana holidays for 2020-2030 against the C++ probe.

Probe key: time/calendars/all -> "botswana"
"""
...
from pquantlib.time.calendars.botswana import Botswana
...
def test_name_matches_cpp(cpp: dict[str, Any]) -> None:
    assert Botswana().name() == cpp["botswana"]["name"]


def test_holidays_match_cpp(cpp: dict[str, Any]) -> None:
    cal = Botswana()
    expected = {
        Date.from_ymd(int(h["d"]), Month(int(h["m"])), int(h["y"])) for h in cpp["botswana"]["holidays"]
    }
    actual: set[Date] = set()
