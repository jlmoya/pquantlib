"""Cross-validate Denmark holidays for 2020-2030 against the C++ probe.

Probe key: time/calendars/all -> "denmark"
"""
...
def test_name_matches_cpp(cpp: dict[str, Any]) -> None:
    assert Denmark().name() == cpp["denmark"]["name"]


def test_holidays_match_cpp(cpp: dict[str, Any]) -> None:
    cal = Denmark()
    expected = {
        Date.from_ymd(int(h["d"]), Month(int(h["m"])), int(h["y"])) for h in cpp["denmark"]["holidays"]
    }
    actual: set[Date] = set()
