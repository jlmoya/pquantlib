"""Cross-validate Iceland holidays for 2020-2030 against the C++ probe.

Probe key: time/calendars/all -> "iceland"
"""
...
def test_name_matches_cpp(cpp: dict[str, Any]) -> None:
    assert Iceland().name() == cpp["iceland"]["name"]


def test_holidays_match_cpp(cpp: dict[str, Any]) -> None:
    cal = Iceland()
    expected = {
        Date.from_ymd(int(h["d"]), Month(int(h["m"])), int(h["y"])) for h in cpp["iceland"]["holidays"]
    }
    actual: set[Date] = set()
