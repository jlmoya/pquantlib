"""Cross-validate Indonesia holidays for 2020-2030 against the C++ probe.

Probe key: time/calendars/all -> "indonesia"
"""
...
def test_name_matches_cpp(cpp: dict[str, Any]) -> None:
    assert Indonesia().name() == cpp["indonesia"]["name"]


def test_holidays_match_cpp(cpp: dict[str, Any]) -> None:
    cal = Indonesia()
    expected = {
        Date.from_ymd(int(h["d"]), Month(int(h["m"])), int(h["y"])) for h in cpp["indonesia"]["holidays"]
    }
    actual: set[Date] = set()
