"""Cross-validate Mexico holidays for 2020-2030 against the C++ probe.

Probe key: time/calendars/all -> "mexico"
"""
...
from pquantlib.time.calendars.mexico import Mexico
...
def test_name_matches_cpp(cpp: dict[str, Any]) -> None:
    assert Mexico().name() == cpp["mexico"]["name"]


def test_holidays_match_cpp(cpp: dict[str, Any]) -> None:
    cal = Mexico()
    expected = {
        Date.from_ymd(int(h["d"]), Month(int(h["m"])), int(h["y"])) for h in cpp["mexico"]["holidays"]
    }
    actual: set[Date] = set()
