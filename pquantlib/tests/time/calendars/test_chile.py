"""Cross-validate Chile holidays for 2020-2030 against the C++ probe.

Probe key: time/calendars/all -> "chile"
"""
...
def test_name_matches_cpp(cpp: dict[str, Any]) -> None:
    assert Chile().name() == cpp["chile"]["name"]


def test_holidays_match_cpp(cpp: dict[str, Any]) -> None:
    cal = Chile()
    expected = {Date.from_ymd(int(h["d"]), Month(int(h["m"])), int(h["y"])) for h in cpp["chile"]["holidays"]}
    actual: set[Date] = set()
