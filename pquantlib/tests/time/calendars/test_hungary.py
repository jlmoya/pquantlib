"""Cross-validate Hungary holidays for 2020-2030 against the C++ probe.

Probe key: time/calendars/all -> "hungary"
"""
...
from pquantlib.time.calendars.hungary import Hungary
...
def test_name_matches_cpp(cpp: dict[str, Any]) -> None:
    assert Hungary().name() == cpp["hungary"]["name"]


def test_holidays_match_cpp(cpp: dict[str, Any]) -> None:
    cal = Hungary()
    expected = {
        Date.from_ymd(int(h["d"]), Month(int(h["m"])), int(h["y"])) for h in cpp["hungary"]["holidays"]
    }
    actual: set[Date] = set()
