"""Cross-validate Singapore (SGX) holidays for 2020-2030 against the C++ probe.

Probe key: time/calendars/all -> "singapore"
"""
...
from pquantlib.time.calendars.singapore import Singapore
...
def test_name_matches_cpp(cpp: dict[str, Any]) -> None:
    assert Singapore().name() == cpp["singapore"]["name"]


def test_holidays_match_cpp(cpp: dict[str, Any]) -> None:
    cal = Singapore()
    expected = {
        Date.from_ymd(int(h["d"]), Month(int(h["m"])), int(h["y"])) for h in cpp["singapore"]["holidays"]
    }
    actual: set[Date] = set()
