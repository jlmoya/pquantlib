"""Cross-validate Sweden holidays for 2020-2030 against the C++ probe.

Probe key: time/calendars/all -> "sweden"
"""
...
def test_name_matches_cpp(cpp: dict[str, Any]) -> None:
    assert Sweden().name() == cpp["sweden"]["name"]


def test_holidays_match_cpp(cpp: dict[str, Any]) -> None:
    cal = Sweden()
    expected = {
        Date.from_ymd(int(h["d"]), Month(int(h["m"])), int(h["y"])) for h in cpp["sweden"]["holidays"]
    }
    actual: set[Date] = set()
