"""Cross-validate Austria holidays for 2020-2030 against the C++ probe.

Probe key: time/calendars/all -> "austria"
"""
...
from pquantlib.time.calendars.austria import Austria
...
def test_name_matches_cpp(cpp: dict[str, Any]) -> None:
    assert Austria().name() == cpp["austria"]["name"]


def test_holidays_match_cpp(cpp: dict[str, Any]) -> None:
    cal = Austria()
    expected = {
        Date.from_ymd(int(h["d"]), Month(int(h["m"])), int(h["y"])) for h in cpp["austria"]["holidays"]
    }
    actual: set[Date] = set()
