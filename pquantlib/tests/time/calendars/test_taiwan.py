"""Cross-validate Taiwan (TSEC) holidays for 2020-2030 against the C++ probe.

Probe key: time/calendars/all -> "taiwan"
"""
...
from pquantlib.time.calendars.taiwan import Taiwan
...
def test_name_matches_cpp(cpp: dict[str, Any]) -> None:
    assert Taiwan().name() == cpp["taiwan"]["name"]


def test_holidays_match_cpp(cpp: dict[str, Any]) -> None:
    cal = Taiwan()
    expected = {
        Date.from_ymd(int(h["d"]), Month(int(h["m"])), int(h["y"])) for h in cpp["taiwan"]["holidays"]
    }
    actual: set[Date] = set()
