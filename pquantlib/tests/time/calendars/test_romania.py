"""Cross-validate Romania holidays for 2020-2030 against the C++ probe.

Probe key: time/calendars/all -> "romania" (default BVB — Bucharest SE).
"""
...
def test_name_matches_cpp(cpp: dict[str, Any]) -> None:
    assert Romania().name() == cpp["romania"]["name"]


def test_holidays_match_cpp(cpp: dict[str, Any]) -> None:
    cal = Romania()
    expected = {
        Date.from_ymd(int(h["d"]), Month(int(h["m"])), int(h["y"])) for h in cpp["romania"]["holidays"]
    }
    actual: set[Date] = set()
