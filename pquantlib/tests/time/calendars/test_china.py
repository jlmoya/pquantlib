"""Cross-validate China holidays for 2020-2030 against the C++ probe.

Probe key: time/calendars/all -> "china"
"""
...
def test_name_matches_cpp(cpp: dict[str, Any]) -> None:
    assert China().name() == cpp["china"]["name"]


def test_holidays_match_cpp(cpp: dict[str, Any]) -> None:
    cal = China()
    expected = {Date.from_ymd(int(h["d"]), Month(int(h["m"])), int(h["y"])) for h in cpp["china"]["holidays"]}
    actual: set[Date] = set()
