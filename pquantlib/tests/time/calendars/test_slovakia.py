"""Cross-validate Slovakia holidays for 2020-2030 against the C++ probe.

Probe key: time/calendars/all -> "slovakia"
"""
...
from pquantlib.time.calendars.slovakia import Slovakia
...
def test_name_matches_cpp(cpp: dict[str, Any]) -> None:
    assert Slovakia().name() == cpp["slovakia"]["name"]


def test_holidays_match_cpp(cpp: dict[str, Any]) -> None:
    cal = Slovakia()
    expected = {
        Date.from_ymd(int(h["d"]), Month(int(h["m"])), int(h["y"])) for h in cpp["slovakia"]["holidays"]
    }
    actual: set[Date] = set()
