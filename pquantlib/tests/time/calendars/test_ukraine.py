"""Cross-validate Ukraine holidays for 2020-2030 against the C++ probe.

Probe key: time/calendars/all -> "ukraine"
"""
...
def test_name_matches_cpp(cpp: dict[str, Any]) -> None:
    assert Ukraine().name() == cpp["ukraine"]["name"]


def test_holidays_match_cpp(cpp: dict[str, Any]) -> None:
    cal = Ukraine()
    expected = {
        Date.from_ymd(int(h["d"]), Month(int(h["m"])), int(h["y"])) for h in cpp["ukraine"]["holidays"]
    }
    actual: set[Date] = set()
