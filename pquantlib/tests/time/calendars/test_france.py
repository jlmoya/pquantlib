"""Cross-validate France holidays for 2020-2030 against the C++ probe.

Probe key: time/calendars/all -> "france"
"""
...
from pquantlib.time.calendars.france import France
...
def test_name_matches_cpp(cpp: dict[str, Any]) -> None:
    assert France().name() == cpp["france"]["name"]


def test_holidays_match_cpp(cpp: dict[str, Any]) -> None:
    cal = France()
    expected = {
        Date.from_ymd(int(h["d"]), Month(int(h["m"])), int(h["y"])) for h in cpp["france"]["holidays"]
    }
    actual: set[Date] = set()
