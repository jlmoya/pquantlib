"""Cross-validate Finland holidays for 2020-2030 against the C++ probe.

Probe key: time/calendars/all -> "finland"
"""
...
from pquantlib.time.calendars.finland import Finland
...
def test_name_matches_cpp(cpp: dict[str, Any]) -> None:
    assert Finland().name() == cpp["finland"]["name"]


def test_holidays_match_cpp(cpp: dict[str, Any]) -> None:
    cal = Finland()
    expected = {
        Date.from_ymd(int(h["d"]), Month(int(h["m"])), int(h["y"])) for h in cpp["finland"]["holidays"]
    }
    actual: set[Date] = set()
