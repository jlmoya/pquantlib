"""Cross-validate Brazil holidays for 2020-2030 against the C++ probe.

Probe key: time/calendars/all -> "brazil"
"""
...
def test_name_matches_cpp(cpp: dict[str, Any]) -> None:
    assert Brazil().name() == cpp["brazil"]["name"]


def test_holidays_match_cpp(cpp: dict[str, Any]) -> None:
    cal = Brazil()
    expected = {
        Date.from_ymd(int(h["d"]), Month(int(h["m"])), int(h["y"])) for h in cpp["brazil"]["holidays"]
    }
    actual: set[Date] = set()
