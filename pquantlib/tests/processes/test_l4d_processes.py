...
class TestHullWhiteForwardProcess:
    ...
    def test_closed_form_helpers(
        ...
        # LOOSE tier: alpha / M_T read forward rates near t1==t2, where L2-B's
        # finite-difference branch (resolved post-Phase-3 in two independent
        # align commits across L4-B + L4-D) diverges by ~1e-12 rel from the
        # C++ analytical short-rate step. Acceptable; not a correctness issue.
        ref = reference_data["hull_white_forward_process"]
        loose(hwf.alpha(1.0), ref["alpha_at_1"])
        loose(hwf.alpha(5.0), ref["alpha_at_5"])
        tight(hwf.B(1.0, 10.0), ref["B_t1_T10"])
        loose(hwf.M_T(0.0, 1.0, 10.0), ref["M_T_s0_t1_T10"])

    def test_drift_diffusion(
        self, hwf: HullWhiteForwardProcess, reference_data: dict[str, Any]
    ) -> None:
        # LOOSE tier: see test_closed_form_helpers — drift reads forward rates.
        ref = reference_data["hull_white_forward_process"]
        x = hwf.x0()
        loose(hwf.drift_1d(1.0, x), ref["drift_t1_x0"])
        tight(hwf.diffusion_1d(1.0, x), ref["diffusion_t1_x0"])

    def test_expectation_variance_stddev(
        self, hwf: HullWhiteForwardProcess, reference_data: dict[str, Any]
    ) -> None:
        # LOOSE tier on expectation: integrates drift over [0, dt], so the
        # forward-rate-near-t1==t2 noise propagates.
        ref = reference_data["hull_white_forward_process"]
        x = hwf.x0()
        loose(hwf.expectation_1d(0.0, x, 1.0), ref["expectation_t0_x0_dt1"])
