"""CMSwapCurveState — constant-maturity-swap market-model curve state.

# C++ parity: ql/models/marketmodels/curvestates/cmswapcurvestate.{hpp,cpp} (v1.42.1).

Stores the yield-curve state parametrized by constant-maturity (CM) swap
rates with a fixed number of spanning forwards. Driven by
``set_on_cm_swap_rates``; discount ratios + annuities follow formula 6.1 of
Joshi-Liesch. Forward / coterminal rates are derived lazily.
"""
...
class CMSwapCurveState(CurveState):
    """Curve state for constant-maturity-swap market models.

    # C++ parity: cmswapcurvestate.hpp CMSwapCurveState.
    """

    def __init__(self, rate_times: list[float], spanning_forwards: int) -> None:
        super().__init__(rate_times)
        n = self._number_of_rates
        self._spanning_fwds = spanning_forwards
        self._first = n
        ...
        self._irr_cm_swap_rates = [0.0] * n
        self._irr_cm_swap_annuities = [last_tau] * n
        self._cot_swap_rates = [0.0] * n
        self._cot_annuities = [last_tau] * n

    def set_on_cm_swap_rates(self, rates: list[float], first_valid_index: int = 0) -> None:
        """Set the state from CM swap rates; recompute discount ratios + annuities.

        # C++ parity: cmswapcurvestate.cpp setOnCMSwapRates (Joshi-Liesch 6.1).
        """
        n = self._number_of_rates
        qassert.require(
            len(rates) == n,
        ...
        for i in range(self._first, n):
            self._cm_swap_rates[i] = rates[i]
        # assume i+1 and do i
        old_annuity_end_index = n
        for i in range(n - 1, self._first, -1):
            # formula 6.1 Joshi Liesch
            end_index = min(i + self._spanning_fwds, n)
            annuity_end_index = min(i + self._spanning_fwds - 1, n)
            self._disc_ratios[i] = (
                self._disc_ratios[end_index]
                + self._cm_swap_rates[i] * self._cm_swap_annuities[i]
            )
            self._cm_swap_annuities[i - 1] = (
                self._cm_swap_annuities[i] + self._disc_ratios[i] * self._rate_taus[i - 1]
            )
            if annuity_end_index < old_annuity_end_index:
                self._cm_swap_annuities[i - 1] -= (
                    self._disc_ratios[old_annuity_end_index]
                    * self._rate_taus[old_annuity_end_index - 1]
                )
            old_annuity_end_index = annuity_end_index
        end_index = min(self._first + self._spanning_fwds, n)
        self._disc_ratios[self._first] = (
            self._disc_ratios[end_index]
            + self._cm_swap_rates[self._first] * self._cm_swap_annuities[self._first]
        )
    ...
    def cm_swap_annuity(self, numeraire: int, i: int, spanning_forwards: int) -> float:
        ...
        qassert.require(self._first <= numeraire <= n, "invalid numeraire")
        qassert.require(self._first <= i <= n, "invalid index")
        if spanning_forwards == self._spanning_fwds:
        ...
        constant_maturity_from_discount_ratios(
            ...
            self._disc_ratios,
            self._rate_taus,
            self._irr_cm_swap_rates,
            self._irr_cm_swap_annuities,
        )
        return self._irr_cm_swap_annuities[i] / self._disc_ratios[numeraire]
    ...
    def cm_swap_rate(self, i: int, spanning_forwards: int) -> float:
        ...
        return self._irr_cm_swap_rates[i]
    ...
    def cm_swap_rates(self, spanning_forwards: int) -> list[float]:
        ...
        return self._irr_cm_swap_rates

    def clone(self) -> CMSwapCurveState:
        new = CMSwapCurveState(self._rate_times, self._spanning_fwds)
        ...
        new._irr_cm_swap_rates = list(self._irr_cm_swap_rates)
        new._irr_cm_swap_annuities = list(self._irr_cm_swap_annuities)
