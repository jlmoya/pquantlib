"""OneStepCoinitialSwaps — the family of coinitial swaps in a single step.

# C++ parity:
# ql/models/marketmodels/products/onestep/onestepcoinitialswaps.{hpp,cpp}
# (v1.42.1).

``lastIndex`` products, one per coinitial swap. In the single step every
...
class OneStepCoinitialSwaps(MultiProductOneStep):
    """The family of coinitial swaps evaluated in a single step.

    # C++ parity: onestepcoinitialswaps.hpp OneStepCoinitialSwaps.
    """
    ...
    def next_time_step(
        ...
        for index_of_time in range(self._last_index):
            ...
            for i in range(index_of_time, self._last_index):
                fixed = cash_flows_generated[i][index_of_time * 2]
                fixed.time_index = index_of_time
                fixed.amount = -self._fixed_rate * self._fixed_accruals[index_of_time]

                floating = cash_flows_generated[i][index_of_time * 2 + 1]
