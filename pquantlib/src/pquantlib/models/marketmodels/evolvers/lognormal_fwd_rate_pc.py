"""LogNormalFwdRatePc — predictor-corrector lognormal forward-rate evolver.

# C++ parity: ql/models/marketmodels/evolvers/lognormalfwdratepc.{hpp,cpp}
# (v1.42.1).

The canonical BGM evolver. Each step advances the log-forwards from ``T1`` to
``T2`` by:

1. computing the drift ``D1`` at ``T1`` (``LMMDriftCalculator``);
2. evolving the log-forwards with ``D1`` + the fixed Ito ``-0.5 sigma^2`` drift
   + the pseudo-root diffusion ``A . Z``;
3. recomputing the drift ``D2`` at the predicted forwards;
4. correcting the log-forwards by ``(D2 - D1) / 2``.
"""
...
class LogNormalFwdRatePc(MarketModelEvolver):
    """Predictor-corrector lognormal forward-rate evolver.

    # C++ parity: lognormalfwdratepc.hpp/.cpp LogNormalFwdRatePc.
    """
    ...
    def __init__(
        ...
        for j in range(steps):
            ...
            # fixed[k] = -0.5 * (A.row(k) . A.row(k)) = -0.5 * variance_k.
    ...
    def _set_forwards(self, forwards: list[float]) -> None:
        # C++ parity: LogNormalFwdRatePc::setForwards.
        qassert.require(
    ...
    def advance_step(self) -> float:
        ...
        for i in range(alive, self._number_of_rates):
            self._log_forwards[i] += (self._drifts2[i] - self._drifts1[i]) / 2.0
            self._forwards[i] = math.exp(self._log_forwards[i]) - self._displacements[i]
