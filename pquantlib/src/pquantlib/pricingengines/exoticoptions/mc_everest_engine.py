...
# C++ parity: ql/experimental/exoticoptions/mceverestengine.{hpp,cpp} (v1.43).
...
class MakeMCEverestEngine:
    """Fluent factory for :class:`MCEverestEngine`.

    # C++ parity: ``MakeMCEverestEngine<RNG, S>``
    # (mceverestengine.hpp:102-124, 210-296).
    ...
    def __init__(
        ...
        # C++ parity: mceverestengine.hpp:210-214.
    ...
    def with_steps(self, steps: int) -> MakeMCEverestEngine:
        self._steps = steps
        return self

    def with_steps_per_year(self, steps: int) -> MakeMCEverestEngine:
        self._steps_per_year = steps
        return self

    def with_brownian_bridge(self, brownian_bridge: bool = True) -> MakeMCEverestEngine:
        self._brownian_bridge = brownian_bridge
        return self

    def with_antithetic_variate(self, b: bool = True) -> MakeMCEverestEngine:
        self._antithetic = b
        return self

    def with_samples(self, samples: int) -> MakeMCEverestEngine:
        qassert.require(self._tolerance is None, "tolerance already set")
        self._samples = samples
        return self

    def with_absolute_tolerance(self, tolerance: float) -> MakeMCEverestEngine:
    ...
    def with_max_samples(self, samples: int) -> MakeMCEverestEngine:
        self._max_samples = samples
        return self

    def with_seed(self, seed: int) -> MakeMCEverestEngine:
        self._seed = seed
        return self

    # ---- terminal ----------------------------------------------------------

    def build(self) -> MCEverestEngine:
        """Construct the engine.

        # C++ parity: ``operator ext::shared_ptr<PricingEngine>()``
        # (mceverestengine.hpp:279-296).
        """
        qassert.require(
    ...
    def __call__(self) -> MCEverestEngine:
        """Mirror the C++ conversion operator to ``shared_ptr<PricingEngine>``."""
        return self.build()


__all__ = ["EverestMultiPathPricer", "MCEverestEngine", "MakeMCEverestEngine"]
