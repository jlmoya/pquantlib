"""MCEuropeanHestonEngine — Monte Carlo European pricing under Heston.

# C++ parity: ql/pricingengines/vanilla/mceuropeanhestonengine.hpp (v1.43) —
# ``template <class RNG, class S, class P> class MCEuropeanHestonEngine``,
# ``class EuropeanHestonPathPricer``, and
# ``template <class RNG, class S, class P> class MakeMCEuropeanHestonEngine``.
...
``MCEuropeanHestonEngine`` has no ``brownianBridge`` parameter and passes
``false`` to its base (mceuropeanhestonengine.hpp:105).

The answer depends on ``HestonProcess``'s discretization scheme, whose C++
default is ``QuadraticExponentialMartingale`` -- *not* the generic Euler
``apply(expectation, stdDeviation * dw)``. See
:class:`~pquantlib.processes.heston_process.Discretization`.
...
and its terminal :meth:`MakeMCEuropeanHestonEngine.engine` checks only the
...
class EuropeanHestonPathPricer(PathPricer[MultiPath]):
    """Terminal-payoff pricer reading the asset leg of a ``MultiPath``.

    # C++ parity: ``EuropeanHestonPathPricer``
    # (mceuropeanhestonengine.hpp:84-94, 214-233).
    ...
    def __call__(self, path: MultiPath) -> float:
        # C++ parity: mceuropeanhestonengine.hpp:225-233.
        asset_path = path[0]
        qassert.require(path.path_size() > 0, "the path cannot be empty")
        return self._payoff(asset_path.back()) * self._discount


class MCEuropeanHestonEngine(MCVanillaEngine[MultiPath]):
    """Monte Carlo European engine on a Heston(-like) process.

    # C++ parity: ``MCEuropeanHestonEngine<RNG, S, P>``
    # (mceuropeanhestonengine.hpp:39-57, 98-131).
    ...
    def path_pricer(self) -> PathPricer[MultiPath]:
        """# C++ parity: ``MCEuropeanHestonEngine::pathPricer``
        # (mceuropeanhestonengine.hpp:113-131)."""
        ...
        return EuropeanHestonPathPricer(
            payoff.option_type(), payoff.strike(), discount
        )


class MakeMCEuropeanHestonEngine:
    """Fluent builder for :class:`MCEuropeanHestonEngine`.

    # C++ parity: ``MakeMCEuropeanHestonEngine<RNG, S, P>``
    # (mceuropeanhestonengine.hpp:60-81, 135-211).
    """
    ...
    def with_steps(self, steps: int) -> MakeMCEuropeanHestonEngine:
        """# C++ parity: ``withSteps`` (mceuropeanhestonengine.hpp:145-152).
    ...
    def with_steps_per_year(self, steps: int) -> MakeMCEuropeanHestonEngine:
        """# C++ parity: ``withStepsPerYear`` (mceuropeanhestonengine.hpp:154-161)."""
        qassert.require(self._steps is None, "number of steps already set")
        self._steps_per_year = steps
        return self

    def with_samples(self, samples: int) -> MakeMCEuropeanHestonEngine:
        """# C++ parity: ``withSamples`` (mceuropeanhestonengine.hpp:163-170)."""
        qassert.require(self._tolerance is None, "tolerance already set")
        self._samples = samples
        return self

    def with_absolute_tolerance(self, tolerance: float) -> MakeMCEuropeanHestonEngine:
        """# C++ parity: ``withAbsoluteTolerance``
        # (mceuropeanhestonengine.hpp:172-182)."""
    ...
    def with_max_samples(self, samples: int) -> MakeMCEuropeanHestonEngine:
        """# C++ parity: ``withMaxSamples`` (mceuropeanhestonengine.hpp:184-189)."""
        self._max_samples = samples
        return self

    def with_seed(self, seed: int) -> MakeMCEuropeanHestonEngine:
        """# C++ parity: ``withSeed`` (mceuropeanhestonengine.hpp:191-196)."""
        self._seed = seed
        return self

    def with_antithetic_variate(self, b: bool = True) -> MakeMCEuropeanHestonEngine:
        """# C++ parity: ``withAntitheticVariate``
        # (mceuropeanhestonengine.hpp:198-203)."""
        self._antithetic = b
        return self

    def engine(self) -> MCEuropeanHestonEngine:
        """# C++ parity: ``operator ext::shared_ptr<PricingEngine>() const``
        # (mceuropeanhestonengine.hpp:205-211).
