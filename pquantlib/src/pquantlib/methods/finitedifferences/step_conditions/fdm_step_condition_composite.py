...
C++ declares two static builders on this class, ``joinConditions`` and
``vanillaComposite``; both are ported below.
"""
...
class FdmStepConditionComposite(StepCondition):
    ...
    def vanilla_composite(
        ...
        """The standard vanilla-option condition set.

        # C++ parity: ``FdmStepConditionComposite::vanillaComposite``
        # (fdmstepconditioncomposite.cpp:81-146).

        Dividends outside ``[ref_date, exercise.last_date()]`` are dropped,
        and the surviving dividend times are pushed as stopping times TWICE:
        once clamped to maturity and once nudged by 1e-5 and re-clamped. That
        second, near-duplicate set is deliberate in C++ — the comment there is
        "smoother convergence behavior with number of time steps" — and a port
        that files it as a redundancy changes the time grid.

        European exercise contributes nothing but is accepted; American and
        Bermudan contribute their respective conditions. Anything else is
        rejected, as in C++.
        """
        stopping_times: list[list[float]] = []
        conditions: list[StepCondition] = []
        ...
        if cash_flow:
            ...
            dividends = [
                d for d in cash_flow if ref_date <= d.date() <= maturity_date
            ]
            dividend_condition = FdmDividendHandler(
                dividends, mesher, ref_date, day_counter, 0
            )
            conditions.append(dividend_condition)

            dividend_times = dividend_condition.dividend_times()
            maturity_time = day_counter.year_fraction(ref_date, maturity_date)
            ...
            stopping_times.append(
                [min(maturity_time, t + 1e-5) for t in dividend_times]
            )
        ...
        if exercise.type() == Exercise.Type.American:
            exercise_start = day_counter.year_fraction(ref_date, exercise.dates()[0])
            conditions.append(
            ...
            conditions.append(bermudan_condition)
