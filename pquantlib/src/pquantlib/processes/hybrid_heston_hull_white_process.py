...
class Discretization:
    ...
    # (hybridhestonhullwhiteprocess.hpp:44). Kept as a plain namespace of int
    # constants rather than an ``IntEnum`` only because the C++ enumerators are
    # unscoped; the values match the C++ declaration order.
    """
...
class HybridHestonHullWhiteProcess(StochasticProcess):
    ...
    def __init__(
        ...
        discretization: int = Discretization.BSMHullWhite,
    ) -> None:
        ...
        self._disc_scheme: int = discretization
